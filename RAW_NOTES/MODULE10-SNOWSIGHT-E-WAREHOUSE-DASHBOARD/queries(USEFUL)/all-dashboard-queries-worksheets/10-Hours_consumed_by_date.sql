WITH END_TIME AS (
SELECT WAREHOUSE_NAME,TIMESTAMP,event_name ,row_number() over (ORDER BY TIMESTAMP ASC) RANK
...
SELECT TO_DATE(START_TIME) START_TIME,WAREHOUSE_NAME, ROUND(SUM(MINUTES)/60,2) as HOURS FROM (
SELECT WAREHOUSE_NAME,START_TIME,END_TIME, MINUTES FROM (
SELECT WAREHOUSE_NAME,START_TIME,END_TIME ,MINUTES ,ROW_NUMBER() OVER (ORDER BY START_TIME ASC) RANK  FROM (
